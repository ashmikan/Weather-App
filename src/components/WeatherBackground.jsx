...
const WeatherBackground = ({ condition }) => {
    ...
    return (
        <div className="fixed inset-0 z-0 overflow-hidden">
            {background === video ? (
                <video autoPlay loop muted className="w-full h-full object-cover opacity-100 pointer-events-none animate-fadeIn">
                    <source src={video} type="video/mp4" />
                </video>
            ) : (
                <img src={background} alt="Weather Background" className="w-full h-full object-cover opacity-20 pointer-events-none animate-fadeIn" />
            )}
